...
function Print-GnomeHelperStatus {
    Print-Header "GNOME Wayland Helper"
    Write-Output "state=not_required"
    Write-Output "helper=gnome_shell_extension"
    Write-Output "required=false"
    Write-Output "installed=false"
    Write-Output "enabled=false"
    Write-Output "healthy=false"
    Write-Output "version=none"
    Write-Output "protocol=none"
    Write-Output "capabilities=none"
    Write-Output "gnome_helper_experimental=false"
    Write-Output "note=GNOME Shell helper is Linux GNOME Wayland only; Windows collector does not probe GNOME."
}
    ...
    for line in handle:
        for key, pattern in patterns.items():
            ...
            if key == "backend" and "Client backend status" not in line:
                continue
            if key == "backend_mismatch" and "Plugin backend hint differs" not in line:
                continue
