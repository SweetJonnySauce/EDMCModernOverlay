...
```
### Example 3: Circle
Circle shapes and explicit `thickness` are EDMCModernOverlay extensions. A
plugin that also targets a legacy overlay must omit those extensions or choose
an appropriate compatibility path.
