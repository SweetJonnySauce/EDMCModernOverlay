...
## Payloads
An atomic message or shape that is sent to an Overlay to be displayed on screen. As of the writing of this wiki the types of Payloads available are text messages (`send_message`), rectangle and circle shapes (`send_shape`), and vector images (`send_raw`). Circle shapes are EDMCModernOverlay extensions; a vector point with `marker: "circle"` remains a distinct vector feature.
