...
| 3 | In progress — manual gate complete, live matrix pending | Fresh Step 3 reconciliation confirmed the prerequisite commits and 156 deterministic tests passed. After separate approval, the user personally completed helper deployment/status: dedicated helper UUID directory clean-replaced; GNOME Wayland (`ubuntu:GNOME`); files installed; extension enabled and ACTIVE; DBus health healthy; protocol 3; `full_helper` presentation enabled. | Collect non-secret evidence for the five live acceptance cases; matching helper readback is mandatory. |
