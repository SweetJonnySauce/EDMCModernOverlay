...
| 2 | Completed | Main-thread review confirmed native-profile-only activation, real-content raster request evidence, windowed/ineligible/failure contracts, generic-neutral dispatch, and preserved legacy/X11/xcompat behavior. Independent required validation: 122 passed; `git diff --check` passed. | Generate the Step 3 task breakdown in a fresh context. |
...
- [x] Step 2 implementation, validation, and scoped commits are reconciled.
