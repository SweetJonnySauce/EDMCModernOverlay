...
- Final environment classification must be based on recorded environment evidence plus selector/status coverage; `true_overlay` requires demonstrated full claimed behavior with no material unresolved limitation in the environment's declared supported play mode, `degraded_overlay` requires demonstrated partial behavior with an explicit weakened guarantee and concrete reason, `unsupported` requires recorded evidence that the required support bar is not met or not implemented, and unvalidated environments must remain deferred rather than being silently classified.
- Fullscreen is not a hard requirement for any backend in this follow-up. If a backend only proves borderless/windowed or windowed reliably, that supported play mode must be stated explicitly rather than treating fullscreen as the default support bar.
- Current GNOME helper evidence must be described honestly: the unresolved stacking blocker was reproduced in borderless/windowed-sized testing, so the remaining limitation cannot be written off as fullscreen-only unless later validation proves that distinction.
