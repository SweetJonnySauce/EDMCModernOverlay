...
- Lifecycle flow:
  ...
  - Enable with `gnome-extensions enable <uuid>`, then accept immediate verification when GNOME reports `ACTIVE` and DBus health responds; otherwise require logout/login before final `ACTIVE` and DBus health verification.
...
- Live detection follow-up:
  - 2026-05-10 evidence showed `scripts/dev_gnome_helper.sh status` can report the helper as installed, `Enabled: Yes`, `State: ACTIVE`, and DBus-health reachable while EDMC preferences still showed `Helper: GNOME Shell extension inactive`.
  - Touch points: client runtime backend probe/status refresh path. The installer lifecycle, Shell extension package, presentation behavior, and `keep_overlay_visible` behavior remain unchanged.
...
- Added `scripts/dev_gnome_helper.sh` as a development-only source-checkout utility for helper install/update/enable/status/disable/uninstall. It follows the same user-local UUID path, source-directory copy, no-sudo, no-global-GNOME-config-change, and install/update/uninstall logout-login rules as the installer, avoids rerunning the full plugin installer during local helper iteration, allows immediate enable verification when GNOME reports `ACTIVE` and DBus health responds, and resolves Snap-confined terminals back to the real account home instead of snap-private data paths.
...
- Recorded Q6 install/enable sequence on 2026-05-09, refined with live evidence on 2026-05-10: detect GNOME Wayland, check prerequisites, ask for user approval, copy files, verify discovery, check global user-extension setting, enable extension, then verify `ACTIVE` and DBus health/protocol compatibility immediately when possible; if discovery/activation/health does not converge, require logout/login and recheck.
...
- Recorded Q6 approval/restart decision on 2026-05-09, refined with live evidence on 2026-05-10: every helper install/config-changing action requires explicit user approval; install/update/uninstall still require logout/login before final trust, while enabling an already discovered helper can be accepted immediately when `gnome-extensions info` reports `ACTIVE` and helper DBus health responds.
