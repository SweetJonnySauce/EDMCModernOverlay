## Goal: Global opacity setting for CMDRs

## Requirements (Initial)

### Goals
- Add a single global opacity preference for CMDRs in the top section of the preferences pane.
- Provide a slider UI: left = fully transparent, right = fully opaque.
- Default to fully opaque (100%).
- Apply changes immediately in the running overlay.
- Represent the value as a percent from 0–100.

### UI/Behavior
- Slider controls a global opacity value for all payloads.
- Per-payload opacity is the opacity used for a payload group when global payload opacity is 100%.
- When global payload opacity is reduced, reduce each per-payload opacity by the same amount down to zero.
- Remember each payload's original per-payload opacity and restore it when global payload opacity returns to 100%.
- Treat payloads/groups without explicit alpha as 100% for the reduction math.
- Apply the reduction to all payload visuals (text, outlines, fills, and group backgrounds).
- Use the same slider styling as the existing dev-only opacity slider.
- Slider shows a percentage with 1% increments.
- Global opacity applies only to payloads and payload groups (including group backgrounds).
- The dev-only “Overlay background opacity” remains separate and continues to control the full-screen background and gridlines.
- Label text: "Overlay payload opacity".
- Placement: immediately below "Chat command to launch Controller" with consistent vertical spacing.

### Persistence
- Store the preference via EDMC config under the key `global_payload_opacity`.
- Mirror the value into `overlay_settings.json` for the client (same behavior as other preferences).

### Implementation Notes (expectations)
- Use the same immediate-update pathway as other preferences (UI callback -> preference update -> overlay config push).
- Ensure the overlay client uses the global opacity uniformly across payload rendering.
